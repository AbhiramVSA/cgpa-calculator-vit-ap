...
function CGPACalculatorContent() {
  ...
  const calculateSemesterGPA = (courses: ImportedCourse[]): number => {
    ...
    const totalGradePoints = validCourses.reduce((sum, course) => {
      ...
      const credits = typeof course.credits === "string" ? Number.parseFloat(course.credits) : course.credits
  ...
  const addCourse = () => {
    ...
    if (!currentCourse.credits || Number.parseFloat(currentCourse.credits) <= 0)
      return alert("Please enter valid credits (greater than 0)")
  ...
  const handleAutomatedImport = async (value?: string) => {
    ...
    try {
      ...
      const processedStudentData: StudentData = {
        ...decoded,
        id: typeof decoded.id === "string" ? Number.parseInt(decoded.id) : decoded.id,
        credits_registered:
          typeof decoded.credits_registered === "string"
            ? Number.parseFloat(decoded.credits_registered)
            : decoded.credits_registered,
        credits_earned:
          typeof decoded.credits_earned === "string"
            ? Number.parseFloat(decoded.credits_earned)
            : decoded.credits_earned,
        cgpa: typeof decoded.cgpa === "string" ? Number.parseFloat(decoded.cgpa) : decoded.cgpa,
        courses: decoded.courses.map((course) => ({
          ...course,
          credits: typeof course.credits === "string" ? Number.parseFloat(course.credits) : course.credits,
        })),
      }
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="relative overflow-hidden">
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <BookOpen className="h-5 w-5 text-primary" /> Manual Calculator
              </CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setCurrentCourse((p: any) => ({ ...p, title: e.target.value }))}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCurrentCourse((p: any) => ({ ...p, credits: e.target.value }))}
                    onKeyPress={handleKeyPress}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setCurrentCourse((p: any) => ({ ...p, grade: value }))}
                  >
                    <SelectTrigger>
          ...
          <Card id="vitap-app-cta" className="relative overflow-hidden">
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Upload className="h-5 w-5 text-primary" /> Academic Report
              </CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              {importSuccess && (
                <div className="flex items-center gap-2 rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm dark:border-green-900/30 dark:bg-green-900/20">
                  <CheckCircle className="h-4 w-4 text-green-600" /> Imported successfully. Opening interactive
                  report...
                </div>
              )}
              {importError && (
        ...
        {courses.length > 0 && (
          <Card className="mt-6">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <BookOpen className="h-5 w-5 text-primary" /> Course Summary
              </CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    <table className="w-full border-collapse">
                      ...
                      <tbody>
                        {courses.map((course, index) => (
                          <tr
                            key={course.id}
                            className={`border-b hover:bg-muted/20 transition-colors ${index % 2 === 0 ? "bg-background" : ""}`}
                          >
